...
//member homepage top section html
function activity_home_top($selected_month = '') {
			//make call to retrieve activities list
			$activities_call = new Activities();
			$activities = $activities_call->activities($selected_month);
		
			$block = "<a href='activity.php?num=".$activities[0]['aID']."' id='picture_link'>
						<div id='picture'>
							<div id='picture_shown_outer'>
								<img src='../images/activities/".str_replace(' ','_',$activities[0]['name']).".jpg' class='activity_picture shown' id='picture_shown'/>
							 </div>
			...
			if($selected_month < date('m') && $selected_month != ''){
				$block .= "<div id='right_arrow' class='arrow'></div>";
				//convert month number to month name
				$month_name = date( 'F', mktime(0, 0, 0, $selected_month) );
				$block .= "<p id='".$selected_month.date('F')."' class='text activity_month'>".$month_name."'s Activities</p>";
			}
			else{
				$block .= "<p id='".date('n').date('F')."' class='text activity_month'>".date('F')."'s Activities</p>";
			}
				$block .= "</div>
						<div id='top_right_activities'>";
								...
								for($i=0;$i<count($activities);$i++){
										...
										if(empty($activities[$i]['reserve_date']))
											...
											$date = DateTime::createFromFormat('Y-m-d', $activities[$i]['reserve_date']);
											$reserve_date = "<p class='activity_reserve'>".$date->format('M j')."</p>";
										}
										if($i==0)
											$class .= ' selected';
									$block .= "<div class='$class' id='".$activities[$i]['aID']."'><p class='activity_name'>".$activities[$i]['name']."</p><p class='activity_type'>".$activities[$i]['type']."</p><a href='calendar.php'>$reserve_date</a></div>";
